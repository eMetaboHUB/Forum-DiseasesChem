...
for( i in 1:n){
  # Prepare matrix
  a <- data[i, ]$COOC
  b <- data[i, ]$TOTAL_PMID_CHEBI - data[i, ]$COOC
  ...
  test <- tryCatch(
    ...
    error = function(e){
      writeLines(paste0(c(data[i, ]$CHEBI, data[i, ]$MESH, e), collapse = "\t"), log)
      return(NULL)
    },
    warning = function(w){
      writeLines(paste(c(data[i, ]$CHEBI, data[i, ]$MESH, w), collapse = "\t"), log)
      return(NULL)
    }
  )
  ...
  if(!is.null(test)){
      ...
      if(is.infinite(test$estimate)){
        ...
        fold_change[i] <- (a/data[i, ]$TOTAL_PMID_CHEBI)/(data[i, ]$TOTAL_PMID_MESH/data[i, ]$TOTAL_PMID)
  }else{
